...
export default function Questionaires() {
  ...
  const getQuestions = () => {
    axios
      .get(server + "api/questionaire/get-questionaire")
      .then((res) => {
        if (res.status == 200) {
  ...
  const deleteItem = (id) => {
    axios(
      `${import.meta.env.VITE_SERVER_URI}api/questionaire/delete-questionaire`,
      {
        method: "delete",
        data: { _id: id },
      }
          ...
          <div className="questionaire__items">
            {Questions &&
              Questions.map((question, key) => {
                return (
                  <div className="question__card" key={key}>
                    <div className="card__header">
                      <h3>
                        {question?.fixtureId?.HomeTeam} vs{" "}
                        {question?.fixtureId?.AwayTeam}
                      </h3>
                      <p>
                        <i className="ri-map-pin-line"></i>{" "}
                        {question?.fixtureId?.Location}{" "}
                      </p>
                      <p>
                        <i className="ri-layout-masonry-line"></i>{" "}
                        {question?.fixtureId?.Group}
                      </p>
                      ...
                      <Button
                        variant="outlined"
                        color="inherit"
                        onClick={() => deleteItem(question._id)}
                      >
                    ...
                    <div className="card__body">
                      ...
                      {question?.questionaires &&
                        question.questionaires.map((ques, elkey) => (
                          <p key={elkey}>
                            {" "}
                            {elkey + 1}. {ques.question}
                          </p>
                        ))}
                      </div>
                    </div>
